...
export default function CalendarPage() {
  ...
  const handleDateClick = (date: Date) => {
    ...
    // Bisa digunakan untuk membuat event baru pada tanggal tersebut
  };

  const handleEventClick = (event: CalendarEvent) => {
    console.log('Event clicked:', event);
    // Bisa digunakan untuk melihat detail event atau edit
  };
  ...
  const getEventColor = (event: CalendarEvent): string => {
    ...
    switch (quadrant) {
      case 'importantUrgent': return '#ef4444'; // red-500
      case 'importantNotUrgent': return '#3b82f6'; // blue-500
      case 'notImportantUrgent': return '#eab308'; // yellow-500
      case 'notImportantNotUrgent': return '#6b7280'; // gray-500
  ...
  const convertTasksToEvents = (tasks: Task[]): CalendarEvent[] => {
    return tasks.map(task => ({
      ...
      end: task.due_date ? new Date(new Date(task.due_date).getTime() + 60 * 60 * 1000) : new Date(),
  ...
  const fetchTasks = async () => {
    ...
    try {
      ...
      const calendarEvents = convertTasksToEvents(tasksData.filter(task => !task.isCompleted));
  ...
  const renderCalendarDays = () => {
    ...
    for (let i = 0; i < startDay; i++) {
      days.push(
        <div 
          key={`empty-${i}`} 
          className="min-h-[120px] p-2 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700"
        ></div>
      );
    }
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        <div
          key={day}
          className={`min-h-[120px] p-2 border border-gray-200 dark:border-gray-700 cursor-pointer transition-all duration-200 hover:bg-gray-50 dark:hover:bg-gray-800 ${
        ...
        >
          <div className={`flex justify-between items-center mb-1 ${isToday ? 'font-bold' : 'font-semibold'}`}>
            <span className={`text-sm ${isToday ? 'text-purple-600 dark:text-purple-400' : isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
              {day}
            </span>
            {isToday && (
            ...
            {dayEvents.slice(0, 3).map(event => {
              ...
              return (
                ...
                >
                  ...
                  <span className="truncate" style={{ color: isDarkMode ? '#f3f4f6' : '#1f2937' }}>
                    {event.title}
                  </span>
                </div>
              );
    ...
    <div className={`min-h-screen transition-colors duration-300 ${
      ...
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <div className={`rounded-2xl overflow-hidden shadow-lg ${
          ...
          <div className="grid grid-cols-7 border-b border-gray-200 dark:border-gray-700">
            {['Min', 'Sen', 'Sel', 'Rab', 'Kam', 'Jum', 'Sab'].map(day => (
              <div 
                key={day} 
                className="p-3 text-center font-medium text-sm text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-700"
              >
                {day}
              </div>
            ))}
        ...
        <div className={`mt-8 p-6 rounded-2xl ${
          ...
          <h3 className={`font-semibold mb-6 text-lg ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
